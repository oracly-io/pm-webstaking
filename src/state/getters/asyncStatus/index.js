...
import { AWAIT_TRANSACTION } from '@actions'
...
export function isCommitingSuccess(state, action, path = []) {
  return isSucceed(state, action, path) && isSucceed(state, AWAIT_TRANSACTION, [action, ...path])
}
