...
import cn from 'clsx'
import { gt, gte, isNegative, isPositive } from '@oracly/pm-libs/calc-utils'
import { fromDecimalERC20 } from '@oracly/pm-libs/calc-utils'
...
import { APPROVE_ACCOUNT_ALLOWANCE, STAKE } from '@actions'
import { SET_PAGE_DATA, BUY_FOR_STAKE } from '@actions'
...
import { getActiveAccountAddress, getBuy4StakePool } from '@state/getters'
import { getLastDepositidByAddress, getPageDataStakingErc20 } from '@state/getters'
import { getLatestbcBlockNumber, getPageDataStake } from '@state/getters'
import { isInitialLoading, isNeverPerformed } from '@state/getters'
import { isCommiting } from '@state/getters'
import { isInsufficientAllowance, isLoading } from '@state/getters'
import { getActiveAccountBalanceERC20 } from '@state/getters'
import { getActualEpochId, hasUnstakedActualDeposit } from '@state/getters'
...
const ProfileForm = (props) => {
  ...
  const handleChange = (e) => {
    ...
    if (gte(maxvalue, newvalue) && /^\d+\.?\d{0,3}$/.test(newvalue)) props.SET_PAGE_DATA({ stake: newvalue })
